...
There has been many previous attempts to interface this sensor out of a Game Boy Camera. The most famous is an [AVR/Arduino port](https://github.com/shimniok/avr-gameboy-cam) which have been coupled with a [java interface](https://www.bot-thoughts.com/2010/04/gameboy-camera-prototyping.html) or a [TI Calculator](https://www.cemetech.net/projects/item.php?id=54). However, these projects use the same C code originating from the [initial 2005 project](http://sophiateam.undrgnd.free.fr/microcontroller/camera/) as main engine.

The project here is a reboot from the initial AVR code with Arduino IDE compatible commands. The code is more simple than previous versions too. It embbeds only the strict necessary instructions to make the sensor work autonomously with a basic autoexposure loop. A Matlab image reader is provided but any other way of sniffing the serial port to get the image pixel data is possible.
