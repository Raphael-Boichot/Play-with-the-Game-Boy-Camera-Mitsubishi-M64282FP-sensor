...
## Ideas
- Embedding the auto-exposure feature, currently made by Octave/Matlab, into the Arduino code, add an SD shield (storing raw binary data on SD) and make a standalone capture device. Would nessecitate an external code to rebuild the images from raw data stored on SD card;
